...
optimizer = optim.Adam([
    {
        "params": model.parameters(), "lr": wconfig.lr*0.9,
    },
    {
...
for epoch in range(wconfig.epochs):
    ...
    for index, (shape_batch, shape_gt_batch, latent_indices) in enumerate(loader):  # in my code I need a step to select a fixed number of sample points if the pcs have different sizes.
        ...
        l_weight = 0.000  # the behavioral mode during training is very sensitive to this parameter. 0/Low values cause model to ignore latent vects. high values reduce model performance
        l_parameters = []
    ...
    z_tensor = torch.stack(latent_vecs, dim=1).transpose(0, 1)  # stack concats along a NEW dimension. torch.cat concats along EXISTING dim
    # print(z_tensor.shape)
    z_cloud_std = torch.norm(torch.std(z_tensor, dim=0), p=2)
    conv1 = model.state_dict()['pdl_net.conv1.weight'].squeeze().cpu().detach().numpy()[:20]  # weights from first conv layer
    z_tensor_samp = z_tensor[:20].cpu().detach().numpy()
    # conv1 = model.state_dict()['pdl_net.conv1.weight']
    ...
    wandb.log({"train-loss": training_loss_epoch, "validation-loss": validation_loss_epoch, "z_cloud_std": z_cloud_std, "conv1_weights": wandb.Image(conv1), "latent_vecs": wandb.Image(z_tensor_samp), "L2 reg": L2})
